  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          IncidentMap(incident: incident, perspective: perspective),
...
class _StatusCard extends StatelessWidget {
  ...
  bool get _isWaiting =>
      incident.stage == IncidentStage.reported ||
      incident.stage == IncidentStage.waitingForResponder;
  bool get _isEnRoute =>
  ...
  String get _title {
    ...
    return switch (perspective) {
      IncidentPerspective.reporter =>
        _isEnRoute ? 'متطوع في الطريق إليك' : 'تم استلام البلاغ',
      IncidentPerspective.nearbyCitizen =>
        incident.nearbyCitizenAcknowledged
            ? 'تم إيقاف التنبيه'
            : 'تنبيه حريق قريب',
      IncidentPerspective.volunteer =>
        _isEnRoute ? 'أنت تستجيب لهذا البلاغ' : 'نداء حريق جديد',
    };
  }
  ...
  String get _statusLabel {
    ...
    if (_isEnRoute) {
      return perspective == IncidentPerspective.reporter
          ? 'متطوع قادم'
          : 'استجابة نشطة';
    }
  ...
  String get _body {
    ...
    return switch (perspective) {
      IncidentPerspective.reporter =>
        _isEnRoute
            ? 'تمت تلبية النداء. يظهر موقع المتطوع ومساره نحو الحريق على الخريطة.'
      ...
      IncidentPerspective.volunteer =>
        _isEnRoute
            ...
            : 'المسار إلى الحادث ظاهر على الخريطة. راجع التفاصيل قبل تلبية النداء.',
    };
  }
    ...
    if (_isWaiting) {
      return [
        ...
        _ActionButton(
          ...
          danger: true,
          onTap: controller.declineVolunteerRequest,
        ),
      ];
    }
    return [
        ...
        children: [
          Expanded(
            child: _ActionButton(
              ...
              danger: true,
              onTap: controller.withdrawVolunteerResponse,
            ),
          ),
          const SizedBox(width: 10),
          Expanded(
...
class IncidentMap extends StatelessWidget {
  ...
  bool get _showRoute =>
      ...
      (perspective == IncidentPerspective.reporter && _isEnRoute);
  bool get _showPerson =>
      perspective != IncidentPerspective.reporter || _isEnRoute;
  bool get _reporterRoute =>
      perspective == IncidentPerspective.reporter && _isEnRoute;
      ...
      child: AspectRatio(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            final fireLeft = _reporterRoute ? 150.0 : 222.0;
            final fireTop = _reporterRoute ? 125.0 : 76.0;
            return Stack(
              fit: StackFit.expand,
              children: [
                const FigmaIcon(
                ...
                if (_showRoute)
                  CustomPaint(
                    painter: VolunteerRoutePainter(
                      reporterLayout: _reporterRoute,
                    ),
                  ),
                if (_showPerson)
                  Positioned(
                ...
                if (_showPerson)
                  Positioned(
                    ...
                    top: 340 * scale,
                    child: _MapChip(
                      label: perspective == IncidentPerspective.reporter
                          ? 'متطوع في الطريق'
                          : 'موقعك الحالي',
                    ),
                  ),
...
class VolunteerRoutePainter extends CustomPainter {
  const VolunteerRoutePainter({required this.reporterLayout});
  final bool reporterLayout;

  @override
  void paint(Canvas canvas, Size size) {
    final end = reporterLayout
        ? Offset(size.width * .56, size.height * .43)
        : Offset(size.width * .77, size.height * .30);
    final path = Path()
  ...
  @override
  bool shouldRepaint(VolunteerRoutePainter oldDelegate) =>
      reporterLayout != oldDelegate.reporterLayout;
}
